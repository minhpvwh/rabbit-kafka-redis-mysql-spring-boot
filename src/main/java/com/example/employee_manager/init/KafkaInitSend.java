package com.example.employee_manager.init;

import com.example.employee_manager.model.Employee;
import com.example.employee_manager.module.KafkaProducer;
import com.example.employee_manager.module.KafkaSenderWithMessageConverter;
...
public class KafkaInitSend {
    ...
    private final KafkaProducer kafkaSenderExample;
    ...
    @Autowired
    public KafkaInitSend(KafkaProducer kafkaSenderExample, KafkaSenderWithMessageConverter messageConverterSender) {
        this.kafkaSenderExample = kafkaSenderExample;
        this.messageConverterSender = messageConverterSender;
    }
    ...
    public void initiateSendingMessage(ApplicationReadyEvent event) throws InterruptedException {
        ...
        Thread.sleep(5000);
        LOG.info("---------------------------------");
        kafkaSenderExample.sendCustomMessage(new Employee("Lucario", "Lucario", "Lucario", "Lucario", "Lucario", "Lucario"), "reflectoring-user");

        Thread.sleep(5000);
        LOG.info("---------------------------------");
        messageConverterSender.sendMessageWithConverter(new GenericMessage<>(new Employee("Pikachu", "Pikachu", "Pikachu", "Pikachu", "Pikachu", "Pikachu")));
    }
}
